document.addEventListener('DOMContentLoaded', function() {
    ...
    const substituteCheckbox = document.getElementById('substituteCheckbox');
...
const substituteTeachers = document.getElementById('substituteTeachers');
const excludedTeachers = document.getElementById('excludedTeachers');
const arrangeSubstituteBtn = document.getElementById('arrangeSubstituteBtn');
...
teacherDropdown.addEventListener('change', function() {
    const teacherName = teacherDropdown.value;
    if (teacherName) {
        substituteSection.style.display = 'inline-block';
        absentTeacher = teacherName;
    } else {
        substituteSection.style.display = 'none';
        substitutePanel.style.display = 'none';
        substituteCheckbox.checked = false;
    }
});

// 代堂复选框变化事件
substituteCheckbox.addEventListener('change', function() {
    substituteMode = this.checked;
    substitutePanel.style.display = this.checked ? 'block' : 'none';
...
function populateSubstituteTeachers() {
    substituteTeachers.innerHTML = '';
    excludedTeachers.innerHTML = '';
    
    // 排除请假教师自己
    const availableTeachers = allTeachers.filter(t => t !== absentTeacher);
    
    availableTeachers.forEach(teacher => {
        const option = document.createElement('option');
        option.value = teacher;
        option.textContent = teacher;
        substituteTeachers.appendChild(option);
    });
}

// 为排除教师列表添加自动选择功能
substituteTeachers.addEventListener('change', function() {
    // 当教师被选中作为代堂候选时，从排除列表中移除
    Array.from(this.selectedOptions).forEach(option => {
        const excludedOption = Array.from(excludedTeachers.options).find(o => o.value === option.value);
        if (excludedOption) {
            excludedTeachers.removeChild(excludedOption);
        }
    });
});

// 编排队堂按钮点击事件
arrangeSubstituteBtn.addEventListener('click', function() {
    // 获取选中的日期
    const selectedDays = Array.from(document.querySelectorAll('input[name="substituteDay"]:checked')).map(cb => parseInt(cb.value));
    ...
    // 获取选中的代堂教师候选名单
    const candidateTeachers = Array.from(substituteTeachers.selectedOptions).map(o => o.value);
    ...
    // 获取排除教师名单
    const excluded = Array.from(excludedTeachers.options).map(o => o.value);
    
    // 为每个选中的日期安排代堂
    let resultHTML = '<h4>代堂安排结果:</h4>';
    
    selectedDays.forEach(day => {
        resultHTML += `<h5>星期${['一','二','三','四','五'][day-1]}:</h5>`;
        
        // 获取请假教师当天的所有课程
        const absentLessons = allLessons.filter(
            lesson => lesson.teacher === absentTeacher && lesson.day === day
        ).sort((a, b) => a.period - b.period);
        
        if (absentLessons.length === 0) {
            resultHTML += `<p>${absentTeacher} 当天没有课程</p>`;
            return;
        }
        ...
        allLessons.forEach(lesson => {
            if (lesson.day === day) {
                dayTeachers[lesson.teacher] = (dayTeachers[lesson.teacher] || 0) + 1;
            }
        });
        
        // 过滤符合条件的代堂教师
        let availableTeachers = candidateTeachers.filter(teacher => 
            !excluded.includes(teacher) && 
            (dayTeachers[teacher] || 0) <= 5
        );
        
        // 按当天课程数排序（课程少的优先）
        availableTeachers.sort((a, b) => (dayTeachers[a] || 0) - (dayTeachers[b] || 0));
        
        if (availableTeachers.length === 0) {
            resultHTML += `<p style="color:red;">没有符合条件的代堂教师</p>`;
            return;
        }
        ...
        absentLessons.forEach(lesson => {
            // 寻找合适的代堂教师
            let substituteTeacher = availableTeachers.find(t => 
                !usedTeachers.has(t) && 
                // 检查代堂教师在该节是否空闲
                !allLessons.some(l => 
                    l.day === day && 
            ...
            if (!substituteTeacher) {
                // 如果没有完全空闲的教师，尝试找当天课程最少的教师
                substituteTeacher = availableTeachers.find(t => 
                    !usedTeachers.has(t)
                );
            }
            
            if (substituteTeacher) {
                usedTeachers.add(substituteTeacher);
                arrangedCount++;
                
                resultHTML += `<li>
                    第${lesson.period}节 (${timeSlots.find(s => s.period === lesson.period.toString()).time}-${timeSlots.find(s => s.period === lesson.period.toString()).endTime}):
                    <strong>${substituteTeacher}</strong> 代堂
                    (${lesson.class} ${lesson.subject} ${lesson.room})
                </li>`;
                
                // 自动将已安排的教师添加到排除列表
                if (!Array.from(excludedTeachers.options).some(o => o.value === substituteTeacher)) {
                    const option = document.createElement('option');
                    option.value = substituteTeacher;
                    option.textContent = substituteTeacher;
                    excludedTeachers.appendChild(option);
                }
            } else {
                resultHTML += `<li style="color:red;">
                    第${lesson.period}节: 找不到合适的代堂教师
                </li>`;
            }
        });
        resultHTML += '</ul>';
        
        resultHTML += `<p>共安排了 ${arrangedCount}/${absentLessons.length} 节课</p>`;
    });
    
    substituteResult.innerHTML = resultHTML;
});
